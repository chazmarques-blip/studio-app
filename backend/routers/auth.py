...
# Wrappers to offload blocking sync I/O (Supabase, bcrypt) off the event loop.
# Without these, every login/signup blocks ALL other async requests for 200–600ms
# (bcrypt verify ~300ms + Supabase HTTP ~100–300ms). Under heavy pipeline load
# this causes cascading timeouts on /auth/login and other endpoints.
...
async def signup(req: SignUpRequest):
    existing = await _run(lambda: supabase.table("users").select("id").eq("email", req.email).execute())
    if existing.data:
        raise HTTPException(status_code=400, detail="Email already registered")

    password_hash = await _run(pwd_context.hash, req.password)
    user_doc = {
        "email": req.email,
    ...
    result = await _run(lambda: supabase.table("users").insert(user_doc).execute())
    user = result.data[0]
    ...
    await _run(lambda: supabase.table("tenants").insert(tenant_doc).execute())
...
async def login(req: SignInRequest):
    result = await _run(lambda: supabase.table("users").select("*").eq("email", req.email).execute())
    if not result.data:
        raise HTTPException(status_code=401, detail="Invalid email or password")
    user = result.data[0]
    if not await _run(pwd_context.verify, req.password, user["password_hash"]):
        raise HTTPException(status_code=401, detail="Invalid email or password")
    token = create_token(user["id"], user["email"])

    # Get extended profile from tenant settings
    tenant = await _run(lambda: supabase.table("tenants").select("settings").eq("owner_id", user["id"]).execute())
    settings = {}
    if tenant.data:
...
async def get_me(user=Depends(get_current_user)):
    result = await _run(lambda: supabase.table("users").select("id, email, name, created_at").eq("id", user["id"]).execute())
    if not result.data:
        raise HTTPException(status_code=404, detail="User not found")
    user_data = result.data[0]
    
    # Adicionar campos padrão
    user_data["full_name"] = user_data.get("name", "")  # Compatibilidade
    user_data["ui_language"] = "pt"  # Default
    user_data["company_name"] = ""
    user_data["onboarding_completed"] = True  # Pular onboarding
...
async def update_profile(update: ProfileUpdate, user=Depends(get_current_user)):
    ...
    if user_fields:
        await _run(lambda: supabase.table("users").update(user_fields).eq("id", user["id"]).execute())
    ...
    if extra_fields:
        tenant = await _run(lambda: supabase.table("tenants").select("id, settings").eq("owner_id", user["id"]).execute())
        if tenant.data:
            settings = tenant.data[0].get("settings", {}) or {}
            settings.update(extra_fields)
            await _run(lambda: supabase.table("tenants").update({"settings": settings}).eq("id", tenant.data[0]["id"]).execute())
...
async def create_tenant(data: TenantCreate, user=Depends(get_current_user)):
    existing = await _run(lambda: supabase.table("tenants").select("*").eq("owner_id", user["id"]).execute())
    if existing.data:
    ...
    result = await _run(lambda: supabase.table("tenants").insert(tenant).execute())
    return result.data[0]


@router.get("/tenants")
async def get_tenant(user=Depends(get_current_user)):
    result = await _run(lambda: supabase.table("tenants").select("*").eq("owner_id", user["id"]).execute())
    if not result.data:
