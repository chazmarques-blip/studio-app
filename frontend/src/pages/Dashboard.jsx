  ...
  if (active && payload?.length) {
    return (
      <div className="rounded-xl border border-white/[0.06] bg-[#0E0E0E]/95 backdrop-blur-xl px-2.5 py-1.5 shadow-xl">
        ...
        <p className="text-xs font-bold text-[#8B5CF6]">{payload[0].value} msgs</p>
      </div>
    );
  }
  return null;
    ...
    <div className="min-h-screen px-3 pt-2 pb-4">
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={1}
          ...
          {[
            ...
            <button key={i} data-testid={`quick-action-${i}`} onClick={() => navigate(a.path)}
              className={`group glass-card flex items-center gap-3 p-3.5 text-left transition-all hover:border-[#8B5CF6]/30 hover:shadow-[0_0_20px_rgba(201,168,76,0.04)] ${a.accent ? 'border-[#8B5CF6]/15' : ''}`}>
              <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl transition-all ${
                a.accent
                  ? 'bg-gradient-to-br from-[#8B5CF6]/20 to-[#8B5CF6]/5 shadow-[0_0_20px_rgba(201,168,76,0.08)] group-hover:shadow-[0_0_25px_rgba(201,168,76,0.15)]'
                  : 'bg-white/[0.03] group-hover:bg-[#8B5CF6]/[0.08]'
              }`}>
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={3}
          ...
          {[
            ...
            <div key={i} data-testid={`stat-${i}`} className="glass-card p-2.5 transition hover:border-[#8B5CF6]/20">
              <div className="mb-1.5 flex items-center justify-between">
                <s.icon size={13} className="text-[#8B5CF6]/70" />
                {s.trend && <ArrowUpRight size={10} className="text-[#8B5CF6]" />}
              </div>
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={4}
            ...
            <div className="h-[100px]" style={{ minWidth: 0 }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={stats?.messages_by_day || []} margin={{ top: 2, right: 2, left: -25, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="count" stroke="#8B5CF6" strokeWidth={1.5} fill="url(#gld)" dot={{ r: 2, fill: '#8B5CF6', strokeWidth: 0 }} activeDot={{ r: 4, fill: '#8B5CF6' }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="glass-card p-3">
            <div className="mb-2 flex items-center justify-between">
              ...
              <button onClick={() => navigate('/crm')} className="text-[9px] text-[#8B5CF6] hover:underline">{t('dashboard.view_all')}</button>
            </div>
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={5}
          ...
          <div className="glass-card p-3">
            <div className="mb-2 flex items-center justify-between">
              ...
              <button onClick={() => navigate('/chat')} className="text-[9px] text-[#8B5CF6] hover:underline">{t('dashboard.view_all')}</button>
            </div>
                ...
                {stats.recent_conversations.map(c => (
                  <div key={c.id} className="flex items-center gap-2.5 rounded-xl bg-white/[0.015] border border-white/[0.04] p-2 transition hover:bg-white/[0.03] hover:border-white/[0.08] cursor-pointer" onClick={() => navigate('/chat')}>
                    ...
                    <div className="text-right shrink-0">
                      <span className={`inline-block rounded-full px-1.5 py-px text-[8px] font-medium ${c.status === 'active' ? 'bg-[#8B5CF6]/15 text-[#8B5CF6]' : 'bg-[#333]/30 text-[#999]'}`}>
                        {c.status}
                      </span>
          ...
          <div className="glass-card p-3">
            <div className="mb-2 flex items-center justify-between">
              ...
              <button onClick={() => navigate('/agents')} className="text-[9px] text-[#8B5CF6] hover:underline">{t('dashboard.view_all')}</button>
            </div>
            {agents.length > 0 ? (
              <div className="space-y-1">
                {agents.map((a) => (
                  <div key={a.id} className="flex items-center gap-2.5 rounded-xl bg-white/[0.015] border border-white/[0.04] p-2 transition hover:bg-white/[0.03] hover:border-white/[0.08]">
                    <div className="relative">
                      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#8B5CF6]/10 ring-1 ring-[#8B5CF6]/15">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-0.5">
                        ...
                        <span className="text-[9px] font-mono text-[#8B5CF6] ml-2">{a.resolved > 0 ? Math.round((a.resolved / Math.max(a.conversations, 1)) * 100) : 0}%</span>
                      </div>
                      <div className="h-1 overflow-hidden rounded-full bg-[#1A1A1A]">
                        <div className="h-full rounded-full bg-gradient-to-r from-[#8B5CF6] to-[#D4B85A] transition-all duration-700" style={{ width: `${a.conversations > 0 ? Math.round((a.resolved / a.conversations) * 100) : 0}%` }} />
                      </div>
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={6}
          data-testid="ai-insights-widget" className="mb-3 glass-card p-3 border-[#8B5CF6]/15">
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className="ml-auto rounded-full bg-[#8B5CF6]/10 px-2 py-0.5 text-[8px] font-semibold text-[#8B5CF6] uppercase tracking-wide">AI</span>
          </div>
          <div className="space-y-1.5">
            {[
              ...
              <div key={i} className="flex items-start gap-2 rounded-xl bg-white/[0.015] border border-white/[0.04] p-2">
                <div className="mt-0.5 h-1 w-1 shrink-0 rounded-full bg-[#8B5CF6]" />
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={7}
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => navigate('/pricing')} className="rounded-full border border-[#8B5CF6]/25 px-2.5 py-0.5 text-[9px] font-medium text-[#8B5CF6] transition hover:bg-[#8B5CF6]/10">
              {t('dashboard.upgrade')}
            </button>
          </div>
          <div className="space-y-1.5">
            <div>
              ...
              <div className="h-1 overflow-hidden rounded-full bg-[#1A1A1A]">
                <div className={`h-full rounded-full transition-all duration-500 ${usagePercent > 80 ? 'bg-[#FF6B6B]' : 'bg-gradient-to-r from-[#8B5CF6] to-[#D4B85A]'}`} style={{ width: `${usagePercent}%` }} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-[9px] mb-0.5">
              ...
              <div className="h-1 overflow-hidden rounded-full bg-[#1A1A1A]">
                <div className="h-full rounded-full bg-gradient-to-r from-[#8B5CF6] to-[#D4B85A] transition-all duration-500" style={{ width: `${Math.min(100, ((stats?.agents_count || 0) / (stats?.agents_limit || 1)) * 100)}%` }} />
              </div>
            </div>
          </div>
        </motion.div>
