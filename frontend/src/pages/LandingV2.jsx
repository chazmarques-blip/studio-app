...
export default function LandingV2() {
  ...
  const agents = [
    ...
    { name: l.ag4_name, role: l.ag4_role, desc: l.ag4_desc, avatar: AVATARS.Emily, channels: ['WhatsApp', 'Telegram', 'Instagram'], skills: [l.skill_personal, l.skill_followup, l.skill_multilang],
      mindset: lang === 'pt' ? 'Assistente pessoal proativa que antecipa necessidades. Gerencia tarefas, organiza rotinas e fornece resumos diários personalizados em qualquer canal.' : lang === 'es' ? 'Asistente personal proactiva que anticipa necesidades. Gestiona tareas, organiza rutinas y brinda resumenes diarios personalizados.' : 'Proactive personal assistant that anticipates needs. Manages tasks, organizes routines, and provides personalized daily summaries across any channel.',
      stats: [{ name: 'Task Completion', pct: 97 }, { name: 'Proactive Alerts', pct: 95 }, { name: 'Routine Accuracy', pct: 99 }, { name: 'User Satisfaction', pct: 98 }],
      metrics: { conversations: '2,341', resolved: '97.8%', avgTime: '< 1s' },
    },
  ];
        ...
        <div className="relative z-10 mx-auto max-w-6xl">
          ...
          <motion.div initial="hidden" animate="visible" variants={fade} custom={3} className="flex justify-center gap-3 mb-12">
            <button onClick={() => navigate('/login?tab=signup')} data-testid="hero-cta-btn"
              className="btn-gold flex items-center gap-1.5 rounded-lg px-4 py-1.5 text-[11px] font-semibold group">
              {l.cta} <ArrowRight size={12} className="group-hover:translate-x-0.5 transition-transform" />
            </button>
            <button data-testid="hero-demo-btn"
              onClick={() => document.getElementById('demo-section')?.scrollIntoView({ behavior: 'smooth' })}
              className="flex items-center gap-1.5 rounded-lg border border-white/[0.07] bg-white/[0.02] px-4 py-1.5 text-[11px] text-[#888] hover:text-white hover:border-white/[0.12] transition-all">
              <Play size={11} /> {l.demo}
            </button>
          </motion.div>
        ...
        <div className="relative z-10 mx-auto max-w-2xl text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fade}>
            ...
            <button onClick={() => navigate('/login?tab=signup')} data-testid="final-cta-btn"
              className="btn-gold rounded-lg px-4 py-1.5 text-[11px] font-semibold inline-flex items-center gap-1.5 group">
              {l.cta} <ArrowRight size={12} className="group-hover:translate-x-0.5 transition-transform" />
            </button>
          </motion.div>
        </div>
