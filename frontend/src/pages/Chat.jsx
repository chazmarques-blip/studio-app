    ...
    return (
      <div key={msg.id} className={`flex ${msg.sender === 'customer' ? 'justify-start' : 'justify-end'}`}>
        <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 ${
          ...
          msg.sender === 'agent' ? 'bg-[#1A1A1A] border-l-2 border-[#C9A84C]/50' :
          'bg-[#1A1A1A] border-l-2 border-[#C9A84C]'
        }`}>
          {msg.sender === 'agent' && (
            <p className="text-[10px] text-[#C9A84C] mb-0.5">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          ...
          {(mediaLoading) && (
            <div className="flex justify-end">
              <div className="rounded-2xl bg-[#1A1A1A] border-l-2 border-[#C9A84C]/50 px-4 py-3">
                <div className="flex items-center gap-2">
                  <div className="h-3 w-3 animate-spin rounded-full border-2 border-[#C9A84C] border-t-transparent" />
                  <span className="text-[10px] text-[#C9A84C]">{lang === 'pt' ? 'Processando...' : 'Processing...'}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {filteredConvos.map((convo) => (
            <button key={convo.id} data-testid={`convo-${convo.id}`} onClick={() => openConversation(convo)}
              ...
              <div className="relative flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-full bg-[#1A1A1A] border border-[#2A2A2A]">
                ...
                <div className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-[#0A0A0A] bg-[#C9A84C]" />
              </div>
              <div className="flex-1 min-w-0">
