  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] px-4 pt-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#1A1A1A] border border-[#2A2A2A]">
      ...
      <div className="mb-4 flex gap-2 overflow-x-auto pb-2">
        {['All', 'WhatsApp', 'Instagram', 'Facebook', 'Telegram'].map((ch, i) => (
          <button
            key={ch}
            data-testid={`filter-${ch.toLowerCase()}`}
      ...
      <div data-testid="chat-empty-state" className="glass-card mt-12 p-8 text-center">
        ...
        <h3 className="mb-2 text-base font-semibold text-white">No conversations yet</h3>
        <p className="text-sm text-[#666666]">
          Connect a channel and start receiving messages from your customers
        </p>
      </div>
    </div>
  );
