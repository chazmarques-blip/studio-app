...
export default function HandoffHuman() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#0A0A0A]">
      ...
      <div className="rounded-lg bg-[#8B5CF6]/10 border border-[#8B5CF6]/30 mx-4 mt-3 px-4 py-2.5 flex items-center gap-2">
        <UserCheck size={16} className="text-[#8B5CF6]" />
        <span className="text-xs text-[#8B5CF6]">{t('handoff.handling')}</span>
      </div>

      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {chatHistory.map((msg, i) => {
          if (msg.sender === 'system') {
            return <div key={i} className="text-center"><span className="rounded-full bg-[#8B5CF6]/10 px-3 py-1 text-[10px] text-[#8B5CF6]">{msg.text}</span></div>;
          }
          const isOutgoing = msg.sender === 'agent' || msg.sender === 'human';
          return (
            <div key={i} className={`flex ${isOutgoing ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${
                msg.sender === 'human' ? 'bg-[#1A1A1A] border-l-2 border-[#8B5CF6]' :
                isOutgoing ? 'bg-[#1A1A1A] border-l-2 border-[#2196F3]' : 'bg-[#222222]'
              }`}>
                {msg.sender === 'human' && <p className="text-[10px] text-[#8B5CF6] mb-1">You (Operator)</p>}
      ...
      <div className="border-t border-[#2A2A2A] px-4 py-3">
        <div className="mb-2 flex gap-2">
          <button className="rounded-lg bg-[#1A1A1A] border border-[#2A2A2A] px-3 py-1.5 text-[10px] text-[#A0A0A0] hover:border-[#8B5CF6]">{t('handoff.return_bot')}</button>
          <button className="rounded-lg bg-[#1A1A1A] border border-[#2A2A2A] px-3 py-1.5 text-[10px] text-[#A0A0A0] hover:border-[#8B5CF6]">{t('handoff.create_lead')}</button>
          <button className="rounded-lg bg-[#1A1A1A] border border-[#2A2A2A] px-3 py-1.5 text-[10px] text-[#A0A0A0] hover:border-[#8B5CF6]">{t('handoff.transfer')}</button>
        </div>
        <div className="flex gap-2">
          <input value={message} onChange={e => setMessage(e.target.value)} placeholder={t('handoff.send_as_operator')}
            className="flex-1 rounded-lg border border-[#2A2A2A] bg-[#1E1E1E] px-4 py-2.5 text-sm text-white placeholder-[#666666] outline-none focus:border-[#8B5CF6]" />
          <button className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#8B5CF6]"><Send size={16} className="text-[#0A0A0A]" /></button>
        </div>
      </div>
    </div>
  );
