    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#0A0A0A] px-4">
      ...
      <div className="relative z-10 w-full max-w-lg">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 data-testid="onboarding-title" className="mb-2 text-2xl font-bold text-white">Choose Your Language</h1>
          <p className="text-sm text-[#A0A0A0]">Select your preferred language for the interface</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {languages.map(lang => (
            <button
              ...
              className={`glass-card group flex flex-col items-center gap-2 p-4 transition-all duration-200 ${
                selectedLang === lang.code
                  ? 'border-[#C9A84C] bg-[#C9A84C]/10 gold-glow'
                  : 'hover:border-[#3A3A3A]'
              }`}
            >
              <span className="text-2xl">{lang.flag}</span>
              <span className={`text-sm font-medium ${selectedLang === lang.code ? 'text-[#C9A84C]' : 'text-[#A0A0A0]'}`}>
                {lang.name}
              </span>
        ...
        <button
          ...
          disabled={loading}
          className="btn-gold mt-8 w-full rounded-xl py-3 text-sm font-semibold disabled:opacity-50"
        >
