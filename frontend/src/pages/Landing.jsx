    ...
    <div className="relative rounded-2xl border border-[#1E1E1E] bg-[#0D0D0D]/90 backdrop-blur-xl overflow-hidden shadow-2xl shadow-black/40">
      <div className="flex items-center gap-3 border-b border-[#1A1A1A] px-4 py-3 bg-[#0A0A0A]">
        <div className="relative">
          <div className="h-9 w-9 rounded-full bg-gradient-to-br from-[#8B5CF6] to-[#A88B3D] flex items-center justify-center">
            <span className="text-xs font-bold text-[#0A0A0A]">S</span>
          </div>
          <div className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-[#8B5CF6] border-2 border-[#0A0A0A]" />
        </div>
        <div className="flex-1">
          <p className="text-sm font-semibold text-white">Sofia</p>
          <p className="text-[10px] text-[#8B5CF6]/70 flex items-center gap-1"><span className="inline-block h-1 w-1 rounded-full bg-[#8B5CF6]" /> Online</p>
        </div>
        <div className="flex items-center gap-1.5">
          <Sparkles size={12} className="text-[#8B5CF6]" />
          <span className="text-[10px] text-[#8B5CF6] font-medium">AI</span>
        </div>
      </div>
        ...
        {msgs.map((m, i) => (
          <div key={i} className={`flex ${m.from === 'user' ? 'justify-end' : 'justify-start'}`} style={{animation: 'slideUp 0.3s ease'}}>
            <div className={`max-w-[82%] rounded-2xl px-3.5 py-2 ${
              m.from === 'user' ? 'bg-[#8B5CF6]/10 border border-[#8B5CF6]/15' : 'bg-[#141414] border border-[#1E1E1E]'
            }`}>
              {m.from === 'agent' && <p className="text-[9px] text-[#8B5CF6]/50 mb-0.5 font-medium">Sofia</p>}
        ...
        {typing && (
          <div className="flex justify-start">
            <div className="rounded-2xl bg-[#141414] border border-[#1E1E1E] px-5 py-3">
              <div className="flex gap-1.5">{[0,150,300].map(d=><div key={d} className="h-1.5 w-1.5 animate-bounce rounded-full bg-[#8B5CF6]/50" style={{animationDelay:`${d}ms`}}/>)}</div>
            </div>
          </div>
        )}
      </div>
      <div className="border-t border-[#1A1A1A] bg-[#0A0A0A] px-3 py-2.5 flex items-center gap-2">
        ...
        <div className="h-8 w-8 rounded-full bg-[#8B5CF6] flex items-center justify-center flex-shrink-0">
    ...
    <div className="min-h-screen bg-[#070707] overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-[#1A1A1A]/60 bg-[#070707]/70 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
          <img src="/logo-studiox.svg" alt="StudioX" className="h-10" />
          <div className="flex items-center gap-3">
            <button data-testid="landing-login-btn" onClick={() => navigate('/login')} className="rounded-lg border border-[#222] px-4 py-2 text-sm text-[#999] hover:border-[#8B5CF6]/40 hover:text-white transition-all">{t('landing.signin')}</button>
      ...
      <section className="relative px-6 pt-24 pb-16 overflow-hidden">
        <div className="absolute top-10 left-1/2 -translate-x-1/2 h-[350px] w-[500px] rounded-full bg-[#8B5CF6]/4 blur-[150px]" />

        <div className="relative z-10 mx-auto max-w-5xl w-full">
          {/* Content centered compact */}
          <div className="text-center mb-8">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-[#8B5CF6]/15 bg-[#8B5CF6]/5 px-3.5 py-1">
              <Zap size={12} className="text-[#8B5CF6]" />
              <span className="text-[10px] font-medium text-[#8B5CF6]/80">{t('landing.badge')}</span>
            </div>

            <h1 className="mb-3 text-3xl font-bold leading-[1.1] tracking-tight text-white sm:text-4xl lg:text-5xl">
              {t('landing.hero_title_1')}{' '}
              <span className="bg-gradient-to-r from-[#8B5CF6] via-[#D9BD6A] to-[#8B5CF6] bg-clip-text text-transparent">{t('landing.hero_title_2')}</span>
            </h1>
            ...
            <div className="flex flex-col gap-2.5 sm:flex-row justify-center mb-6">
              ...
              <button className="rounded-xl border border-[#1E1E1E] bg-[#0D0D0D] px-7 py-2.5 text-sm text-[#888] hover:border-[#8B5CF6]/30 hover:text-white transition-all">{t('landing.watch_demo')}</button>
            </div>
      ...
      <section className="py-16 px-6 border-t border-[#141414]">
        <div className="mx-auto max-w-5xl">
          <div className="text-center mb-8">
            <p className="text-[10px] font-semibold text-[#8B5CF6] tracking-widest uppercase mb-2">Omnichannel</p>
            <h2 className="mb-2 text-2xl font-bold text-white leading-tight sm:text-3xl">
              {lang === 'pt' ? 'Todos os canais. ' : 'Every channel. '}
              <span className="text-[#8B5CF6]">{lang === 'pt' ? 'Um painel.' : 'One dashboard.'}</span>
            </h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {channels.map((ch, i) => (
              <div key={i} className="rounded-xl border border-[#141414] bg-[#0C0C0C] p-4 text-center hover:border-[#8B5CF6]/20 hover:bg-[#0E0E0E] transition-all group cursor-default">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#8B5CF6]/8 mx-auto mb-3 group-hover:bg-[#8B5CF6]/12 transition">
      ...
      <section className="py-16 px-6 border-t border-[#141414] bg-[#0A0A0A]">
        <div className="mx-auto max-w-5xl">
          <div className="text-center mb-10">
            <p className="text-[10px] font-semibold text-[#8B5CF6] tracking-widest uppercase mb-2">Marketplace</p>
            ...
            {agents.map((a, i) => (
              <button key={i} onClick={() => setActiveAgent(i)}
                className={`rounded-xl border p-4 text-left transition-all duration-300 ${
                  activeAgent === i ? 'border-[#8B5CF6]/30 bg-[#8B5CF6]/5' : 'border-[#141414] bg-[#0C0C0C] hover:border-[#1E1E1E]'
                }`}>
                <div className="h-10 w-10 rounded-lg flex items-center justify-center text-sm font-bold mb-2 bg-[#8B5CF6]/10 text-[#8B5CF6]">{a.name[0]}</div>
                <p className="text-sm font-semibold text-white mb-0.5">{a.name}</p>
                <p className="text-[10px] text-[#B0B0B0]">{a.role} · {a.desc}</p>
                {activeAgent === i && <div className="mt-2 h-0.5 rounded-full bg-gradient-to-r from-[#8B5CF6] to-transparent" />}
              </button>
            ))}
          </div>
          <div className="text-center">
            <button onClick={() => navigate('/login?tab=signup')} className="inline-flex items-center gap-2 text-sm text-[#8B5CF6]/80 hover:text-[#8B5CF6] transition group">
      ...
      <section className="mx-auto max-w-5xl px-6 py-16">
        <div className="text-center mb-10">
          <p className="text-[10px] font-semibold text-[#8B5CF6] tracking-widest uppercase mb-2">{lang === 'pt' ? 'Funcionalidades' : 'Features'}</p>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f, i) => (
            <div key={i} className="group rounded-xl border border-[#141414] bg-[#0C0C0C] p-5 transition-all duration-300 hover:border-[#8B5CF6]/20 hover:bg-[#0E0E0E]">
              <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-lg bg-[#8B5CF6]/8 group-hover:bg-[#8B5CF6]/12 transition">
                <f.icon size={18} className="text-[#8B5CF6]" />
              </div>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 border-t border-[#141414]">
        <div className="text-center mb-8">
          <p className="text-[10px] font-semibold text-[#8B5CF6] tracking-widest uppercase mb-2">Pricing</p>
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-[#1E1E1E] bg-[#0D0D0D] p-1">
            <button data-testid="billing-annual" onClick={() => setBillingAnnual(true)}
              className={`rounded-full px-4 py-1.5 text-xs font-medium transition ${billingAnnual ? 'bg-[#8B5CF6] text-[#0A0A0A]' : 'text-[#999] hover:text-white'}`}>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-xl border border-[#141414] bg-[#0C0C0C] flex flex-col p-5 hover:border-[#1E1E1E] transition">
            ...
            <ul className="mb-5 flex-1 space-y-2 text-[12px] text-[#777]">
              {['f1','f2','f3','f4'].map(k=><li key={k} className="flex items-center gap-2"><Check size={12} className="text-[#8B5CF6] flex-shrink-0"/>{t(`landing.plan_free_${k}`)}</li>)}
            </ul>
          ...
          <div className="rounded-xl border border-[#141414] bg-[#0C0C0C] relative flex flex-col p-5 hover:border-[#1E1E1E] transition">
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full border border-[#8B5CF6]/30 bg-[#0D0D0D] px-3 py-0.5 text-[10px] font-medium text-[#8B5CF6]">{t('landing.plan_starter_badge')}</div>
            ...
            <div className="mb-4"><span className="text-2xl font-bold text-[#8B5CF6]">{t('landing.plan_starter_price')}</span><span className="text-xs text-[#B0B0B0]">{t('landing.plan_starter_period')}</span></div>
            <ul className="mb-5 flex-1 space-y-2 text-[12px] text-[#777]">
              {['f1','f2','f3','f4','f5'].map(k=><li key={k} className="flex items-center gap-2"><Check size={12} className="text-[#8B5CF6] flex-shrink-0"/>{t(`landing.plan_starter_${k}`)}</li>)}
            </ul>
            <button className="btn-gold w-full rounded-lg py-2 text-sm font-semibold">{t('landing.plan_starter_cta')}</button>
          </div>
          {/* PRO - Most Popular */}
          <div className="rounded-xl border border-[#8B5CF6]/25 bg-[#0C0C0C] relative flex flex-col p-5 shadow-lg shadow-[#8B5CF6]/3">
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#8B5CF6] px-3 py-0.5 text-[10px] font-semibold text-[#0A0A0A]">{t('landing.plan_pro_badge')}</div>
            ...
            <ul className="mb-5 flex-1 space-y-2 text-[12px] text-[#777]">
              {['f1','f2','f3','f4','f5'].map(k=><li key={k} className="flex items-center gap-2"><Check size={12} className="text-[#8B5CF6] flex-shrink-0"/>{t(`landing.plan_pro_${k}`)}</li>)}
            </ul>
            <button className="btn-gold w-full rounded-lg py-2 text-sm font-semibold">{t('landing.plan_pro_cta')}</button>
          </div>
          {/* ENTERPRISE */}
          <div className="rounded-xl border border-[#141414] bg-[#0C0C0C] relative flex flex-col p-5 hover:border-[#1E1E1E] transition">
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full border border-[#8B5CF6]/30 bg-[#0D0D0D] px-3 py-0.5 text-[10px] font-medium text-[#8B5CF6]">{t('landing.plan_enterprise_badge')}</div>
            ...
            <ul className="mb-5 flex-1 space-y-2 text-[12px] text-[#777]">
              {['f1','f2','f3','f4','f5','f6','f7','f8'].map(k=><li key={k} className="flex items-center gap-2"><Check size={12} className="text-[#8B5CF6] flex-shrink-0"/>{t(`landing.plan_enterprise_${k}`)}</li>)}
            </ul>
      ...
      <section className="py-16 px-6 border-t border-[#141414] relative">
        <div className="absolute inset-0 overflow-hidden"><div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-[200px] w-[400px] rounded-full bg-[#8B5CF6]/3 blur-[100px]" /></div>
      ...
      <footer className="border-t border-[#141414] px-6 py-6">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-3 sm:flex-row">
          ...
          <p className="text-[10px] text-[#999]">2026 StudioX</p>
        </div>
      </footer>
    </div>
  );
