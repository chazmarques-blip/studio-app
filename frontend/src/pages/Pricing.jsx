...
export default function Pricing() {
  ...
  return (
    <div className="min-h-screen px-4 pt-5 pb-24">
      ...
      {/* Current Plan Card */}
      {stats && (
        <div className="mb-5 rounded-xl border border-[#8B5CF6]/20 bg-gradient-to-br from-[#8B5CF6]/5 to-transparent p-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <p className="text-xs text-[#8B5CF6]/70">Current Plan</p>
          ...
          return (
            <div key={plan.key} data-testid={`plan-card-${plan.key}`}
              className={`rounded-xl border p-4 transition ${
                isCurrent ? 'border-[#8B5CF6]/30 bg-[#8B5CF6]/5' : 'border-[#1A1A1A] bg-[#0D0D0D] hover:border-[#222]'
              }`}>
              <div className="flex items-start gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl flex-shrink-0" style={{ backgroundColor: isCurrent ? 'rgba(201,168,76,0.15)' : '#141414' }}>
                  {plan.key === 'free' ? <Zap size={16} className="text-[#888]" /> :
                   plan.key === 'starter' ? <Sparkles size={16} className="text-[#8B5CF6]" /> :
                   plan.key === 'pro' ? <Crown size={16} className="text-[#8B5CF6]" /> :
              ...
              <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1">
                {features.map((f, fi) => (
                  <span key={fi} className="flex items-center gap-1 text-[11px] text-[#999]">
                    <Check size={10} className="text-[#8B5CF6]" />{f}
                  </span>
                ))}
              </div>
              ...
              <div className="mt-3">
                {isCurrent ? (
                  <div className="text-center py-2 text-xs text-[#8B5CF6] font-medium">Your current plan</div>
                ) : isUpgrade ? (
                  ...
                  <button data-testid={`downgrade-${plan.key}`} onClick={() => handleUpgrade(plan.key)} disabled={upgrading === plan.key}
                    className="w-full rounded-lg border border-[#1E1E1E] py-2.5 text-sm text-[#999] hover:border-[#8B5CF6]/30 hover:text-white transition disabled:opacity-50">
