    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#0A0A0A] px-4">
      ...
      <div className="relative z-10 w-full max-w-lg">
        ...
        <div className="mb-8 text-center">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#C9A84C]/10"><Languages size={28} className="text-[#C9A84C]" /></div>
        ...
        <div className="space-y-3 mb-6">
          <button data-testid="mode-fixed" onClick={() => setMode('fixed')}
            className={`glass-card w-full flex items-center gap-4 p-5 text-left transition-all ${mode === 'fixed' ? 'border-[#C9A84C] bg-[#C9A84C]/10 gold-glow' : 'hover:border-[#3A3A3A]'}`}>
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#C9A84C]/10"><Globe size={20} className="text-[#C9A84C]" /></div>
            <div className="flex-1">
              <p className={`text-sm font-semibold ${mode === 'fixed' ? 'text-[#C9A84C]' : 'text-white'}`}>{t('onboarding.fixed_lang')}</p>
              <p className="text-xs text-[#666666]">{t('onboarding.fixed_lang_desc')}</p>
            </div>
            {mode === 'fixed' && <Check size={18} className="text-[#C9A84C]" />}
          </button>
          <button data-testid="mode-auto" onClick={() => setMode('auto_detect')}
            className={`glass-card w-full flex items-center gap-4 p-5 text-left transition-all ${mode === 'auto_detect' ? 'border-[#C9A84C] bg-[#C9A84C]/10 gold-glow' : 'hover:border-[#3A3A3A]'}`}>
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#C9A84C]/10"><Sparkles size={20} className="text-[#C9A84C]" /></div>
            <div className="flex-1">
              <p className={`text-sm font-semibold ${mode === 'auto_detect' ? 'text-[#C9A84C]' : 'text-white'}`}>{t('onboarding.auto_detect')}</p>
              <p className="text-xs text-[#666666]">{t('onboarding.auto_detect_desc')}</p>
            </div>
            {mode === 'auto_detect' && <Check size={18} className="text-[#C9A84C]" />}
          </button>
        </div>
        {mode === 'fixed' && (
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              {langs.map(l => (
                <button key={l.code} onClick={() => setFixedLang(l.code)}
                  className={`glass-card flex items-center gap-2 p-3 text-sm ${fixedLang === l.code ? 'border-[#C9A84C] text-[#C9A84C]' : 'text-[#A0A0A0]'}`}>
        ...
        {mode === 'auto_detect' && (
          <div className="mb-6">
            ...
            <select value={fallbackLang} onChange={e => setFallbackLang(e.target.value)}
              className="w-full rounded-lg border border-[#2A2A2A] bg-[#1E1E1E] px-4 py-2.5 text-sm text-white outline-none focus:border-[#C9A84C]">
