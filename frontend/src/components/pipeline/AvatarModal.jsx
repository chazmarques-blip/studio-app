...
export function AvatarModal({ ctx }) {
  ...
  return (
    <div 
      className={`fixed inset-0 ${zIndexOverride || 'z-50'} bg-black/80 flex items-center justify-center p-4`} 
      style={{pointerEvents: 'auto'}}
      onClick={(e) => { 
        console.log('🔴 Backdrop clicked');
        if (!generatingAvatar && !applyingClothing) resetAvatarModal(); 
      }}
    >
      <div 
        data-testid="avatar-modal" 
        className="w-full max-w-xl rounded-2xl border border-[#8B5CF6]/20 bg-[#0D0D0D] overflow-hidden max-h-[85vh] flex flex-col" 
        style={{pointerEvents: 'auto'}}
        onClick={e => {
