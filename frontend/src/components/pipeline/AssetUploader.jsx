...
function AssetUploader({ assets, onAssetsChange }) {
  ...
  const UploadColumn = ({ type, items, inputRef, color, icon: Icon, label, testId }) => (
    <div>
      <p className="text-[8px] text-[#999] uppercase tracking-wider mb-1">{label}</p>
      ...
      {items.length > 0 ? (
        <div className="flex gap-1.5 flex-wrap items-center">
          ...
          <button onClick={() => inputRef.current?.click()} disabled={uploading}
            ...
            <Upload size={10} className="text-[#999]" />
          </button>
        </div>
      ) : (
  ...
  return (
    <div data-testid="asset-uploader" className="space-y-3">
      <label className="text-[9px] text-[#999] uppercase tracking-wider block">{t('studio.product_images')}</label>
