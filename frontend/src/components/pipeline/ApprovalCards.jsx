...
function CopyApproval({ data, onApprove }) {
  ...
  return (
    <div data-testid="copy-approval" className="mt-3 space-y-2.5 bg-amber-500/5 rounded-lg p-3 border border-amber-500/20">
      ...
      <p className="text-[9px] text-[#888]">Lee recomendou a <span className="text-[#8B5CF6] font-bold">Variacao {autoSel}</span></p>
      <div className="flex gap-2">
        {[1, 2, 3].map(n => (
          <button key={n} data-testid={`select-copy-${n}`} onClick={() => setSelected(n)}
            className={`flex-1 rounded-lg py-2.5 text-[11px] font-semibold border-2 transition-all ${selected === n ? 'border-[#8B5CF6] bg-[#8B5CF6]/15 text-[#8B5CF6] shadow-[0_0_10px_rgba(201,168,76,0.15)]' : 'border-[#222] text-[#666] hover:text-white hover:border-[#333]'}`}>
            {n === autoSel ? `Var ${n} *` : `Variacao ${n}`}
          </button>
        ))}
      </div>
      <button data-testid="approve-copy-btn" onClick={handleApprove} disabled={submitting}
        className="w-full rounded-lg bg-gradient-to-r from-[#8B5CF6] to-[#D4B85A] py-2.5 text-[12px] font-bold text-black hover:opacity-90 transition-all flex items-center justify-center gap-2 disabled:opacity-50 shadow-[0_0_20px_rgba(201,168,76,0.2)]">
        ...
        <div className="grid grid-cols-3 gap-2">
          {images.map((url, i) => (
            <button key={i} onClick={() => setLightboxIdx(i)}
              className="rounded-lg overflow-hidden border-2 border-[#222] hover:border-[#8B5CF6]/50 transition relative group">
          ...
          <div className="flex gap-1.5 flex-1">
            {[1, 2, 3].map(n => (
              <button key={n} onClick={() => setSelections(prev => ({ ...prev, [p]: n }))}
                className={`flex-1 rounded-lg py-2 text-[10px] font-semibold border-2 transition-all ${(selections[p] || 1) === n ? 'border-[#8B5CF6] bg-[#8B5CF6]/15 text-[#8B5CF6]' : 'border-[#222] text-[#666] hover:text-white'}`}>
