  ...
  return (
    <div data-testid={`step-card-${step}`} className={`rounded-xl border transition-all duration-300 ${
      ...
      <button onClick={onToggle} className="w-full px-3 py-2.5 flex items-center gap-2.5">
        ...
        {data?.elapsed_ms && <span className="text-[8px] text-[#888] shrink-0 bg-[#111] px-1.5 py-0.5 rounded">{(data.elapsed_ms / 1000).toFixed(1)}s</span>}
        {(data?.output || isFailed || requiresUpgrade || isWaitingAudio) && (expanded ? <ChevronUp size={14} className="text-[#888]" /> : <ChevronDown size={14} className="text-[#888]" />)}
      </button>
  ...
  return (
    <div className="px-3 pb-3 border-t border-[#151515]">
      ...
      {hasVideo && data.video_url && (
        <div className="mt-2">
          <p className="text-[9px] text-[#999] uppercase tracking-wider mb-1.5 flex items-center gap-1"><Film size={10} className="text-red-400" /> {t('studio.video_commercial')}</p>
      ...
      {hasImages && (
        <div className="mt-2">
          <p className="text-[9px] text-[#999] uppercase tracking-wider mb-1.5">{t('studio.images_generated')}</p>
          ...
          {data.platform_variants && Object.keys(data.platform_variants).length > 0 && (
            <div className="mt-2 flex items-center gap-1.5 flex-wrap">
              <span className="text-[8px] text-[#999]">{t('studio.formats')}:</span>
    ...
    <div data-testid="audio-approval-panel" className="mt-3 rounded-xl bg-gradient-to-b from-purple-500/5 to-transparent border border-purple-500/20 p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div>
          ...
          <p className="text-[9px] text-[#999]">{t('studio.audio_preapproval_desc') || 'Review the narration before generating the video (saves credits)'}</p>
        </div>
      </div>
      ...
      {data?.audio_preview_url && (
        <div
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[8px] text-[#999]">{t('studio.ai_recommended') || 'AI-recommended voice for this campaign'}</p>
          </div>
          <Volume2 size={12} className={audioPlaying ? 'text-purple-400 animate-pulse' : 'text-[#777]'} />
        </div>
      )}
      ...
      {alternatives.length > 0 && (
        <div className="mb-3">
          <p className="text-[8px] text-[#888] uppercase tracking-wider font-semibold mb-1.5 flex items-center gap-1">
            ...
            {alternatives.map(alt => (
              <div key={alt.voice_id} data-testid={`alt-voice-${alt.voice_id}`}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[8px] text-[#999] truncate">
      ...
      <div className="mb-3">
        <textarea data-testid="audio-feedback-input"
          ...
          className="w-full bg-[#0A0A0A] border border-[#1A1A1A] rounded-lg p-2.5 text-[10px] text-[#ccc] placeholder-[#666] resize-none focus:outline-none focus:border-purple-500/40"
          rows={2}
        />
      </div>
