...
- **Voice Consistency (A+C) — Sora Character Lock + ElevenLabs Fallback** (2026-04-24): (a) Descoberto que endpoint OpenAI mudou em 2026: `/v1/sora/characters` → `/v1/videos/characters` (multipart + requires 2-4s clip); `_register_sora_character_http` reescrito para baixar vídeo, cortar 3s com ffmpeg, upload multipart; agora retorna tupla `(char_id, error_reason)` com razões específicas (moderation_rejected / face_detected / duration_invalid / etc); (b) Auto-register em background thread após cada cena Sora renderizada com sucesso (`_auto_register_sora_characters_for_scene` em production.py L276-366) — trava voz+aparência na primeira cena do personagem; (c) Erros "sticky" (moderation/face) memorizados em `sora_character_lock_error` para evitar retry storm; (d) Novo endpoint `GET /api/studio/projects/{id}/voice-status` unifica Sora Character Lock + ElevenLabs voice_map com status "locked" / "fallback_only" / "unassigned"; (e) Frontend: novo `VoiceStatusPanel.jsx` renderiza no step 2 (Personagens) com header de contagem, banner explicativo, botão "Travar Vozes do Sora" (auto-register all), cards por personagem mostrando badge de status, sora_character_id truncado, voz ElevenLabs fallback, botões preview / retry / unlock. Verificado em JONAS E O PEIXE GRANDE: 1/11 personagens TRAVADOS (Mensageiro @ cena #30), 10 em fallback_only (rejeitados por moderação — rostos bíblicos realistas). Sistema funcionando dentro das limitações reais da API OpenAI. ⚠️ Não testado via testing agent.
